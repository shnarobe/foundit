  ...
  <div class="row"><?php echo $column_left; ?>
    ...
    <div id="content" class="<?php echo $class; ?>"><?php echo $content_top; ?>
	  ...
	  <link rel="stylesheet" media="screen and (min-width: 1px) and (max-width: 700px)" href="<?php echo $mobile_stylesheet; ?>" />
              <?php if (isset($stylesheet_theme)) { ?>
              <link rel="stylesheet" media="screen and (min-width: 701px) and (max-width: 99999px)" href="<?php echo $stylesheet_theme; ?>" />
              <?php } ?>             
	  <?php } else { ?>
	  <link rel="stylesheet" href="<?php echo $stylesheet; ?>" />
              <?php if ($stylesheet_theme) { ?>
              <link rel="stylesheet" media="screen and (min-width: 701px) and (max-width: 99999px)" href="<?php echo $stylesheet_theme; ?>" />
              <?php } ?>  
	  <?php } ?>
	  ...
	  <div id="onepagecheckoutconfirm">
		<div id="onepagecheckout-csl">
		  <?php if (!$logged && $login_module) { ?>
		  <div class="onepagecheckoutmid" id="login-box">
			<div id="checkout">
			  <div class="onepage-ch-heading"><i class="fa fa-sign-in"></i> <?php echo $text_checkout_option; ?></div>
			  <div class="onepage-ch-content"><?php echo $login; ?></div>
			</div>
			<div class="or"><?php echo $text_or; ?></div>
		  </div>
		  <?php } ?>
		  <div class="onepagecheckoutleft">
			<div id="payment-address">
			  <div class="onepage-ch-heading"><i class="fa fa-user"></i> <?php echo !$logged ? $text_checkout_account : $text_checkout_payment_address; ?></div>
			  <div class="onepage-ch-content"><?php echo isset($guest) ? $guest : $payment_address; ?></div>
			</div>
			<?php if ($shipping_required) { ?>
			<div id="shipping-address">
			  <div class="onepage-ch-heading"><i class="fa fa-user"></i> <?php echo $text_checkout_shipping_address; ?></div>
			  <div class="onepage-ch-content"><?php echo isset($shipping_address) ? $shipping_address : ''; ?></div>
			</div>
			<?php } ?>
		  </div>
		  <div class="onepagecheckoutright">
			<?php if ($shipping_required) { ?>
			<div id="shipping-method">
			  <div class="onepage-ch-heading"><i class="fa fa-truck"></i> <?php echo $text_checkout_shipping_method; ?></div>
			  <div class="onepage-ch-content"></div>
			</div>
			<?php } ?>
			<div id="payment-method" <?php echo !$payment_module ? ' hidden' : ''; ?>>
			  <div class="onepage-ch-heading"><i class="fa fa-credit-card"></i> <?php echo $text_checkout_payment_method; ?></div>
			  <div class="onepage-ch-content"></div>
			</div>
		  </div>
			<div class="onepagecheckoutright">
			  <div id="voucher">
				<div class="onepage-ch-content" style="border:none; padding: 0px;overflow: hidden;"><?php echo $voucher; ?></div>
			  </div>
			</div>

			<div class="onepagecheckoutright">
			  <?php if ($cart_module) { ?>
                          <div class="onepage-ch-heading"><i class="fa fa-shopping-cart"></i> <?php echo $text_checkout_shopping_cart; ?></div>
			  <div id="onepagecheckoutcart">
				<div class="onepage-ch-content" style="border:none; padding: 0px;"></div>
			  </div>
			  <?php } ?>
		...
		<div class="onepagecheckoutmid">
		  <div id="terms">
			<div id="terms-bottom" class="onepage-ch-content text-right"><?php echo $terms; ?></div>
		  </div>
		</div>
	  </div>
...
<?php if ($custom_css_heading_background) { ?>
<style type="text/css">
.onepage-ch-heading {
    background: <?php echo $custom_css_heading_background; ?>;
}
</style>
<?php } ?>

<?php if ($custom_css_heading_font) { ?>
<style type="text/css">
.onepage-ch-heading {
    color: <?php echo $custom_css_heading_font; ?>;
}
</style>
<?php } ?>

<?php if (!empty($custom_css_border_radius)&&($custom_css_border_radius == 'sharp')) { ?>
<style type="text/css">
.onepage-ch-heading {
...
<?php if (!empty($custom_css_border_color)) { ?>
<style type="text/css">
.onepage-ch-content {
  border-color: <?php echo $custom_css_border_color; ?>;
}
</style>
<?php } ?>
...
function validateComment() {
	$.ajax({
		url: 'index.php?route=quickcheckout/cart/validate',
		type: 'post',
		data: $('#order-comment select, #order-comment input[type=\'radio\']:checked, #order-comment input[type=\'checkbox\']:checked, #order-comment textarea'),
		dataType: 'json',
		cache: false,
...
$(document).on('click', '#button-payment-method', function() {
	...
	$('#button-payment-method').after('<i class="fa fa-refresh fa-spin"></i>');
...
function reloadPaymentMethod() {
	$.ajax({
		...
		success: function(html) {
			...
			$('#payment-method .onepage-ch-content').html(html);
	...
	function reloadShippingMethod(type) {
		...
		$.ajax({
			...
			success: function(html) {
				...
				$('#shipping-method .onepage-ch-content').html(html);
			},
			<?php if ($debug) { ?>
			error: function(xhr, ajaxOptions, thrownError) {
				alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
	...
	$.ajax({
		...
		beforeSend: function() {
			<?php if ($confirmation_page) { ?>
				...
				<?php if ($slide_effect) { ?>
				$('#onepagecheckoutconfirm').slideUp('slow');
				<?php } else { ?>
				$('#onepagecheckoutconfirm').html('<div class="text-center"><i class="fa fa-refresh fa-spin fa-5x"></i></div>');
				<?php } ?>
		...
		success: function(html) {
			<?php if ($confirmation_page) { ?>
				...
				$('#onepagecheckoutconfirm').hide().html(html);
				
				<?php if (!$auto_submit) { ?>
					<?php if ($slide_effect) { ?>
					$('#onepagecheckoutconfirm').slideDown('slow');
					<?php } else { ?>
					$('#onepagecheckoutconfirm').show();
					<?php } ?>
				<?php } else { ?>
				$('#onepagecheckoutconfirm').after('<div class="text-center"><i class="fa fa-refresh fa-spin fa-5x"></i></div>');
				<?php } ?>
			<?php } else { ?>
				$('#terms .terms').hide();
				...
				<?php if ($auto_submit) { ?>
				$('#payment').hide().after('<div class="text-center"><i class="fa fa-refresh fa-spin fa-5x"></i></div>');
				<?php } ?>
...
function loadCart() {
	$.ajax({
		...
		beforeSend: function() {
			...
			moduleLoad($('#onepagecheckoutcart'), <?php echo $loading_display; ?>);
		},
		success: function(html) {
			moduleLoaded($('#onepagecheckoutcart'), <?php echo $loading_display; ?>);
			
			$('#onepagecheckoutcart .onepage-ch-content').html(html);
		},
		<?php if ($debug) { ?>
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
$(document).on('click', '#button-coupon', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#button-coupon').after('<i class="fa fa-refresh fa-spin"></i>');
		},
		complete: function() {
			$('#button-coupon').prop('disabled', false);
			$('#coupon-content .fa-refresh').remove();
		},
		success: function(json) {
			$('.alert').remove();
...
$(document).on('click', '#button-voucher', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#button-voucher').after('<i class="fa fa-refresh fa-spin"></i>');
		},
		complete: function() {
			$('#button-voucher').prop('disabled', false);
			$('#voucher-content .fa-refresh').remove();
		},
		success: function(json) {
			$('.alert').remove();
...
$(document).on('click', '#button-reward', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#button-reward').after('<i class="fa fa-refresh fa-spin"></i>');
		},
		complete: function() {
			$('#button-reward').prop('disabled', false);
			$('#reward-content .fa-refresh').remove();
		},
		success: function(json) {
			$('.alert').remove();
...
$(document).on('click', '.button-update', function() {
	$.ajax({
		...
		type: 'post',
		data: $('#onepagecheckoutcart :input'),
		dataType: 'json',
		cache: false,
		beforeSend: function() {
			$('#onepagecheckoutcart .button-update').prop('disabled', true);
		},
		success: function(json) {
			if (json['redirect']) {
				location = json['redirect'];
...
$(document).on('click', '.button-remove', function() {
	...
	$.ajax({
		...
		cache: false,
		beforeSend: function() {
			$('#onepagecheckoutcart .button-remove').prop('disabled', true);
		},
		success: function(json) {
			if (json['redirect']) {
				location = json['redirect'];
