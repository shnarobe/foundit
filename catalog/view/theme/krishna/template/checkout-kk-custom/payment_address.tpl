<form class="form-horizontal" id="payment-form-k">
  <?php if ($addresses) { ?>
  <div class="radio">
    <label>
  ...
  <div class="buttons clearfix" id="hide-payment" style="display:none;">
...
$('input[name=\'payment_address\']').on('change', function() {
	if (this.value == 'new') {
		...
		$('#payment-new').show();
		$('#hide-payment').show();//show continue button if user clicks new address 
	} else {
		$('#payment-existing').show();
		$('#payment-new').hide();
		$('#hide-payment').hide();//hide continue if user does not click new address
	}
});
//--></script>
<script type="text/javascript"><!--
...
$('#collapse-payment-address select[name=\'country_id\']').trigger('change');
		...
		//if the user selects a different payment/billing address
		/*then the various panels should be updated starting from the payment address panel onwards.
		first update the panel where the change is made before calling the other panels.
		1. on change the value of the payment address select field is updated
		2. via ajax in onekone the checkout/payment-address/save is called.
		3. now if shipping required, a call is then made to checkout/shipping_address
		4. on completion of the ajax call, the form in checkout/shipping-address is then serialized and put in local storage
		5. for later access.*/
$('#payment-existing select').on('change',function(){
	//on chage of payment address 
	//save new payment address but use existing values for shipping address
	paymentformk=$("#payment-form-k").serialize();
	//console.log(this.value);
	onekone(paymentformk);
	//update local storage
	sessionStorage.removeItem("payment-address-k");
	sessionStorage.setItem("payment-address-k",paymentformk);
	//get local storage for shipping method
	arg2=sessionStorage.getItem("shipping-address-k");
	twoktwo(arg2);
	arg3=sessionStorage.getItem("shipping-method-k");
	threekthree(arg3);
