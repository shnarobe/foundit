...
<div class="delivery-details"><?php //var_dump(debug_backtrace());?>
                                    ...
                                    <div class="shipping-method-select">
                                        ...
                                        <?php } else if(!empty($product['shipping_methods']) && !empty($product['shipping_methods']['locations'])) { ?>
                                            ...
                                            <?php foreach($product['shipping_methods']['locations'] as $location) { ?>
                                                <div class="radio">
                                                    <label>
                                                        <input type="radio" name="fixed_shipping_method[<?php echo $product['product_id']; ?>]" value="<?php echo $location['mspl.location_id']; ?>" <?php echo ($location === reset($product['shipping_methods']['locations'])) ? 'checked="checked"' : ''; ?> />
...
<div class="buttons row">
    ...
    <div class="col-sm-4 shipping-total">
        <div class="pull-right">
            ...
            <input style=" display:none;" type="button" value="<?php echo $button_continue; ?>" id="ms-button-shipping-method" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary" />
        </div>
    </div>
    ...
    $(function() {
        function getTotalShippingCost() {
            ...
            $.ajax({
				...
				complete:function(){
					customk();//this automatic call replaces the need to click the continue button to move from shipping method to payment method
        ...
        $(document).on('change', '#collapse-shipping-method input[type="radio"]', getTotalShippingCost);//whenever there is a changes in selection
		//the above is called, therefore by removing the continue button, i can make the call as if the continue button was clicked,.
                    ...
                    if (json['redirect']) {
                        ...
                        $.ajax({
                            ...
                            success: function(html) {
                                ...
                                $('#collapse-payment-method').parent().find('.panel-heading .panel-title').html('<a href="#collapse-payment-method" data-toggle="" data-parent="#accordion" class="accordion-toggle"><?php echo $text_checkout_payment_method; ?> <i class="fa fa-caret-down"></i></a>');
