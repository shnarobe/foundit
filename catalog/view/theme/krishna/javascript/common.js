...
function krishna(e){
	window.location=e;
	//$("#carousel-feature").carousel('prev');
...
$(document).ready(function() {
	...
	/* Search:1-changed header element to nav so search value is picked up */
	...
	var url_k = $('base').attr('href') + 'index.php?route=custom/search';

            $( "#search-kris" ).autocomplete({
               source: url_k,//make ajax call to custom/search
				...
				select:function(event,ui){
					...
					//$('#search-kris').attr("value",ui.item.label);

				//	if (value) {
						url_1 += '&product_id=' + encodeURIComponent(ui.item.pid);
					//}
	...
	//onhover for product display boxes
$("div.product-layout").mouseover(function(){
		//if()
		$(this).find("button").attr("class","btn btn-info");
        $(this).find(".button-group").attr("class","btn-group").css("display", "inline-block");
		
    });
    $("div.product-layout").mouseout(function(){
        $(this).find(".btn-group").attr("class","button-group").css("display", "none");
    });
