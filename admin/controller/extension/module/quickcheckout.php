...
class ControllerExtensionModuleQuickCheckout extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			//var_dump($this->request->post);
			//the field names from the post array serve as the keys for entry into the settings table
			...
			if (!isset($this->request->get['continue'])) {
				...
				$this->response->redirect($this->url->link('extension/module/quickcheckout', 'token=' . $this->session->data['token'] . '&store_id=' . $store_id, true));
			}
		}
		...
		$this->load->model('localisation/country');
                ...
                $data['testzone'] = $this->model_localisation_zone->getZonesByCountryId($this->config->get('config_country_id'));
		...
		//var_dump($setting);
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/quickcheckout', 'token=' . $this->session->data['token'], true)
   		);

                $data['action'] = $this->url->link('extension/module/quickcheckout', 'token=' . $this->session->data['token'] . '&store_id=' . $store_id, true);
                $data['continue'] = $this->url->link('extension/module/quickcheckout', 'token=' . $this->session->data['token'] . '&continue=1&store_id=' . $store_id, true);
	...
	public function install(){
		...
		$data = array(
			...
			'quickcheckout_survey_required'		=> '0',
                        'quickcheckout_survey_text'		=> '{'.$this->config->get('config_language_id').':"Do you like our Quick Checkout module?"}',
                        'quickcheckout_survey_answers'		=> array(
                                                                        array(
                                                                        $this->config->get('config_language_id') => 'Yes'),
                                                                        array(
                                                                        $this->config->get('config_language_id') => 'No')
                        )
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/quickcheckout')) {
