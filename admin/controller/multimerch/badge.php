<?php
class ControllerMultimerchBadge extends ControllerMultimerchBase {
	private $error = array();
	...
	public function getTableData() {
		...
		foreach ($results as $result) {
			...
			// actions
			$actions = "";
			$actions .= "<a class='ms-button ms-button-edit' href='" . $this->url->link('multimerch/badge/update', 'token=' . $this->session->data['token'] . '&badge_id=' . $result['badge_id'], 'SSL') . "' title='".$this->language->get('text_edit')."'></a>";
			$actions .= "<a class='ms-button ms-button-delete' href='" . $this->url->link('multimerch/badge/delete', 'token=' . $this->session->data['token'] . '&badge_id=' . $result['badge_id'], 'SSL') . "' title='".$this->language->get('text_delete')."'></a>";
	...
	public function index() {
		...
		$this->data['breadcrumbs'] = $this->MsLoader->MsHelper->admSetBreadcrumbs(array(
			...
			array(
				...
				'href' => $this->url->link('multimerch/badge', '', 'SSL'),
			)
		));
		
		$this->data['insert'] = $this->url->link('multimerch/badge/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('multimerch/badge/delete', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view("multiseller/badge.tpl", $this->data));
	}
	...
	public function insert() {
		...
		$this->data['cancel'] = $this->url->link('multimerch/badge', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['footer'] = $this->load->controller('common/footer');
		$this->data['header'] = $this->load->controller('common/header');
		$this->response->setOutput($this->load->view("multiseller/badge-form.tpl", $this->data));
	}
	...
	public function update() {
		...
		$badge = $this->MsLoader->MsBadge->getBadges(array('single' => 1, 'badge_id' => $this->request->get['badge_id']), array());
	...
	public function delete() { 
		...
		$this->response->redirect($this->url->link('multimerch/badge', 'token=' . $this->session->data['token'], 'SSL'));
	}
	...
	private function getEditForm()
	{
		...
		if (!isset($this->request->get['badge_id'])) {
			$this->data['action'] = $this->url->link('multimerch/badge/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('multimerch/badge/update', 'token=' . $this->session->data['token'] . '&badge_id=' . $this->request->get['badge_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('multimerch/badge', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function jxSave() {
		...
		foreach ($data['description'] as $language_id => $value) {
			if ((utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 32)) {
				$json['errors']['badge[description]['.$language_id.'][name]'] = $this->language->get('ms_error_badge_name');
			}
		}
